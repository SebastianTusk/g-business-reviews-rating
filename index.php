...
class google_business_reviews_rating
{
	...
	private function admin_current()
	{
		...
		if (isset($_GET['page']) && preg_match('/^google[\s_-]?(?:my[\s_-]?)?business[\s_-]?reviews?[\s_-]?rating(?:[\s_-]?settings?)?$/i', $_GET['page']))
		{
			return TRUE;
		}
...
';
	...
	public function admin_ajax()
	{
		...
		$stylesheet = (isset($post['stylesheet']) && is_numeric($post['stylesheet']) && $post['stylesheet'] >= 0 && $post['stylesheet'] <= 2) ? intval($post['stylesheet']) : 1;
		$javascript = (isset($post['javascript']) && is_numeric($post['javascript']) && $post['javascript'] >= 0 && $post['javascript'] <= 2) ? intval($post['javascript']) : 1;
		$custom_styles = (isset($post['custom_styles']) && mb_strlen($post['custom_styles']) > 2 && !preg_match('/<\?(?:php|=)/i', $post['custom_styles'])) ? $post['custom_styles'] : NULL;
...
';
	...
	private function review_item($data = NULL, $options = NULL, $type = 'all')
	{
		...
		case 'review':
			...
			$set_excerpt = (is_numeric($review_text_excerpt_length) && mb_strlen(strip_tags($data['text'])) > 20 && $review_text_excerpt_length < round(mb_strlen(strip_tags($data['text'])) * 1.1));
			...
			if ($set_excerpt)
			{
				$html .= preg_replace('/(\r\n|\r|\n)+/', '<br>' . PHP_EOL . '				', preg_replace('/^(.{' . $review_text_excerpt_length . '}[^\s]{0,20})(.*)$/uis', '<span class="review-snippet">$1</span> <span class="review-more-placeholder">… ' . esc_html($more) . '</span><span class="review-full-text">$2</span>', esc_html(strip_tags($data['text']))));
			}
			else
			{
				$html .= preg_replace('/(\r\n|\r|\n)+/', '<br>' . PHP_EOL . '				', esc_html(strip_tags($data['text'])));
			}
